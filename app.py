...
def student_dashboard():
    ...
    # プロジェクト配属
    projects = ProjectAssignment.query.filter_by(student_id=current_user.id).all()
...
def student_attendance():
    ...
    if request.method == "POST":
        ...
        record = AttendanceRecord(
            student_id=current_user.id, date=attendance_date, status=status, note=note
        )
...
def student_channel_post():
    ...
    post = ChannelPost(
        student_id=current_user.id, channel_name=channel_name, content=content
    )
    db.session.add(post)
    db.session.commit()
...
@app.route("/student/project", methods=["POST"])
@login_required
def student_project():
    if current_user.role != "student":
        return redirect(url_for("index"))
    ...
    # 既に登録がある場合は更新
    existing_project = ProjectAssignment.query.filter_by(
    ...
    db.session.commit()
    flash("ゼミ情報を登録しました", "success")
    return redirect(url_for("student_dashboard"))


@app.route("/student/project/delete/<int:project_id>", methods=["POST"])
@login_required
...
def student_portfolio():
    ...
    # 自分のポートフォリオ一覧
    portfolios = (
        Portfolio.query.filter_by(student_id=current_user.id)
...
def student_chat():
    ...
    if teacher_id:
        ...
        Message.query.filter_by(
            sender_id=teacher_id, receiver_id=current_user.id, is_read=False
...
def student_send_message():
    ...
    message = Message(
        sender_id=current_user.id, receiver_id=teacher_id, content=content
    )
...
def teacher_dashboard():
    ...
    return render_template(
        "teacher/dashboard.html", students=students, unread_count=unread_count
    )
...
def teacher_student_detail(student_id):
    ...
    # チャンネル投稿
    posts = (
        ChannelPost.query.filter_by(student_id=student_id)
        .order_by(ChannelPost.posted_at.desc())
        .all()
    )

    # プロジェクト配属
    projects = ProjectAssignment.query.filter_by(student_id=student_id).all()

    # ポートフォリオ
    portfolios = (
        Portfolio.query.filter_by(student_id=student_id)
        .order_by(Portfolio.created_at.desc())
        .all()
    )
...
def teacher_send_message():
    ...
    message = Message(
        sender_id=current_user.id, receiver_id=student_id, content=content
    )
    db.session.add(message)
    db.session.commit()
